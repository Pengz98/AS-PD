...
class ModelNet40Data(Dataset):
	def __init__(
		...
		if class_choice is None:
			...
			index = np.where(labels == class_choice)[0]
			self.data, self.labels = data[index], labels[index]
...
class RegistrationData(Dataset):
	def __init__(self, algorithm, data_class=ModelNet40Data(), is_testing=False):
		...
		if self.algorithm == 'PCRNet' or self.algorithm == 'iPCRNet':
			...
			# self.transforms = PCRNetTransform(len(data_class), angle_range=45, translation_range=0)	#zp
