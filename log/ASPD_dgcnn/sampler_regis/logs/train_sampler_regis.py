...
def parse_args():
    ...
    parser.add_argument('--coarse_log_dir', type=str, default='sampler_regis_32', help='other pretrained model')
    parser.add_argument('--log_dir', type=str, default='sampler_regis_16_512')
    ...
    parser.add_argument('--uni_train', type=bool, default=True)
            ...
            if args.sample_model=='SNET':
                ...
                while n_sample<=512:
                    if args.skip_projection:
                        ...
                        subset_t = proj_subset_t[:,:n_sample,:]
                        subset_s = proj_subset_s[:,:n_sample,:]
                    samp_data = (subset_t, subset_s, igt)
                    task_loss, pcrnet_loss_info = task_criterion(samp_data, task_model)
                    sample_loss_t = sample_criterion(template, pred_subset_t)
                    sample_loss_s = sample_criterion(source, pred_subset_s)
