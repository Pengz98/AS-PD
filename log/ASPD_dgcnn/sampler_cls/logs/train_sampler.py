...
def parse_args():
    ...
    parser.add_argument('--coarse_log_dir', type=str, default='sampler_cls_32_ol=0.01', help='other pretrained model')
    parser.add_argument('--log_dir', type=str, default='sampler_cls_16_512_ol=0.01')
    ...
    parser.add_argument('--beta', type=float, default=0.01, help='default=1')
    ...
    parser.add_argument('--initial_temp', type=float, default=0.01, help='default=0.1')

    parser.add_argument('--partial_train', type=bool, default=False)

    parser.add_argument('--save_last', type=bool, default=True)
    parser.add_argument('--save_anchor', type=bool, default=False)

    parser.add_argument('--uni_train', type=bool, default=False)
    parser.add_argument('--progressive_uni', type=bool, default=False)
    parser.add_argument('--infinite_uni', type=bool, default=False)
...
def load_checkpoints(model, checkpoints_dir, source_checkpoints_dir=None):
    try:
        ...
        #     model_state_dict = checkpoint['model_state_dict']
        #
        #     # del params related to attention module
        #     extra_param = []
        #     for param_tensor in model_state_dict:
        #         if 'SELayer' in param_tensor or 'sa1' in param_tensor:
        #             extra_param.append(param_tensor)
        #     for param in extra_param:
        #         model_state_dict.pop(param)
        #
...
def main(args):
    ...
    if args.use_large_dataset:
        ...
        # train_dataset_list = [train_dataset_list[-1]]
    ...
    if args.sample_model=='SNET':
        if args.progressive_version:
            num_out_points = args.n_point
        else:
            num_out_points = args.n_sample
        sampler = MODEL.get_model(num_out_points=num_out_points).to(cur_device)
    else:
    ...
    n_input_list = np.linspace(512,1536,9).astype('int')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        if args.progressive_uni:
            step = args.epoch//len(n_sample_list) // 2
            pro_n_sample_list = n_sample_list[:(1+epoch//step)]
            log_string('epoch %d, n_sample_list: %s' % (epoch, str(pro_n_sample_list)))
        ...
        for dataset_id in range(len(train_dataset_list)):
            ...
            for batch_id, (points, label) in tqdm(enumerate(data_loader), total=len(data_loader)):
                ...
                # print('param num of sampelr:', num_param)

                if args.sample_model=='SNET':
                    if args.progressive_version:
                        n_sample = 32
                        while n_sample<=1024:
                            subset = pred_subset[:, :n_sample, :]
                            pred = task_model(subset.transpose(2,1))

                            # num_param = 0
                            # for name, param in task_model.named_parameters():
                            #     if 'sampler' not in name:
                            #         num_param += param.numel()
                            # print('param num of task model:', num_param)

                            task_loss = task_criterion(pred, label)
                            sample_loss = sample_criterion(points, pred_subset)
                            if n_sample==32:
        ...
        with torch.no_grad():
            ...
            for dataset_id in range(len(test_dataset_list)):
                ...
                for batch_id, (points, label) in tqdm(enumerate(data_loader), total=len(data_loader)):
                    ...
                    # if args.uni_train:
                    #     if args.progressive_uni:
                    #         n_sample = pro_n_sample_list[batch_id % len(pro_n_sample_list)]
                    #     else:
                    #         # n_sample = np.random.choice(n_sample_list)
                    #         n_sample = n_sample_list[batch_id%len(n_sample_list)]
                    # else:
            ...
            log_string('epoch %d, avg test acc (16-512 points) %.5f' % (epoch, test_mean_acc))
        ...
        epoch_anchors = [5,10,20,50]
        if args.save_anchor and (epoch+1) in epoch_anchors:
            log_string('Save model...')
            savepath = str(checkpoints_dir) + '/epoch_' + str(epoch+1) + '.pth'
            log_string('Saving at %s' % savepath)
            state = {
                'epoch': epoch,
                'model_state_dict': task_model.state_dict(),
