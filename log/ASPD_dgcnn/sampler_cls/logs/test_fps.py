...
def parse_args():
    ...
    parser.add_argument('--class_choice', type=int, default=8)
...
def main(args):
    ...
    with torch.no_grad():
            ...
            for dataset_id in range(len(test_dataset_list)):
                ...
                for batch_id, (points, label) in tqdm(enumerate(data_loader), total=len(data_loader)):
                    ...
                    if args.visu:
                        ...
                        for b in range(simplified_subset_np.shape[0]):
                            visu_pc_w_dis(points_np[b], 'input points')
                            visu_pc_w_dis(pred_subset_np[b], 'sampled points')

                            savetxt_pc_w_dis(points_np[b], 'FPS_input_chair' + str(b), 'FPS_64')
                            savetxt_pc_w_dis(pred_subset_np[b], 'FPS_sample_chair' + str(b), 'FPS_64')
                        exit()
        ...
        np.save('result/new/fps.npy', result)
